...
public class StatementCreatorUtils {
  ...
  static {
    ...
    typeMethodMappings.put(Time.class, "getTime");
    javaTypeToSqlTypeMap.put(Timestamp.class, Types.TIMESTAMP);
    typeMethodMappings.put(Timestamp.class, "getTimestamp");
  ...
  private static boolean isDateValue(Class<?> paramType) {
    return (java.util.Date.class.isAssignableFrom(paramType)
        && !(Date.class.isAssignableFrom(paramType)
            || Time.class.isAssignableFrom(paramType)
            || Timestamp.class.isAssignableFrom(paramType)));
  }
    ...
    if (sqlType == Types.VARCHAR || sqlType == Types.LONGVARCHAR) {
      ...
      if (param instanceof java.util.Date) {
        if (param instanceof Date) {
          ...
          ps.setDate(parameterIndex, new Date(((java.util.Date) param).getTime()));
        }
      } else if (param instanceof Calendar) {
        Calendar cal = (Calendar) param;
        ps.setDate(parameterIndex, new Date(cal.getTime().getTime()), cal);
      } else {
      ...
      if (param instanceof java.util.Date) {
        if (param instanceof Time) {
          ps.setTime(parameterIndex, (Time) param);
        } else {
          ps.setTime(parameterIndex, new Time(((java.util.Date) param).getTime()));
        }
      } else if (param instanceof Calendar) {
        Calendar cal = (Calendar) param;
        ps.setTime(parameterIndex, new Time(cal.getTime().getTime()), cal);
      } else {
      ...
      if (param instanceof java.util.Date) {
        if (param instanceof Timestamp) {
          ...
          ps.setTimestamp(parameterIndex, new Timestamp(((java.util.Date) param).getTime()));
        }
      } else if (param instanceof Calendar) {
        Calendar cal = (Calendar) param;
        ps.setTimestamp(parameterIndex, new Timestamp(cal.getTime().getTime()), cal);
      } else {
